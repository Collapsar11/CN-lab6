...
void *process_client(void *thread_info)
{
    ...
    while (true)
    {
        ...
        {
            std::lock_guard<std::mutex> lock(cout_mutex);
            std::cout << "  Send message: " << recv_packet.get_message()
                      << "to " << recv_packet.get_id() << std::endl;
        }
            send_message(client_socket, list_id, recv_packet.get_id(),
                         recv_packet.get_message(), *server);
            break;
